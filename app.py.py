...
# FUNÇÃO DE EXTRAÇÃO INTELIGENTE ATUALIZADA
def extrair_dados_ficha(texto_ficha):
    ...
    # 2. CAPTURA DA EMPRESA / RAZÃO SOCIAL
...
def gerar_pdf(pasta_script, dados_empresa):
    ...
    # Margens compactas para garantir 1 página única
    ...
    estilo_texto = ParagraphStyle("Texto", parent=styles["Normal"], fontSize=9, leading=12, textColor=colors.HexColor("#444444"))
    estilo_topico = ParagraphStyle("Topico", parent=styles["Normal"], fontSize=9, leading=12.5, textColor=colors.HexColor("#333333"))
    ...
    logo_topo = carregar_imagem(caminho_logo(pasta_script, LOGO_CABECALHO), altura=58)
    linha_divisoria = LinhaVertical(altura=52, cor="#B0B0B0", largura_linha=1)
